...
function createRegiment(req, res) { 
    ...
    newRegiment.save( () => {
        Athlete.findById(req.params.id, (err, athlete) => { // find athlete to push the new regiment ID into the reference ARR
            console.log(athlete)
            athlete.regiments.push(newRegiment._id) // acutaly push into ARR
            athlete.save((err) => { // Saves the athlete model, with new reference to regiment
                console.log('A new regiment was added to the DB, which a user can reference')
                if (err) return res.redirect('/athletes')
                res.rediret('/athletes/home')
            })
        })
    })
